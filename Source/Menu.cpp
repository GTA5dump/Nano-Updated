...
int MenuNotification(char* text, char* text2, char* Subject)
{
	...
	UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG("CHAR_PEGASUS_DELIVERY", "CHAR_PEGASUS_DELIVERY", true, 7, text2, Subject, 1.0f, "BASE");
...
void main() {
	YTDNotification();
	MenuNotification("Press F4  to open the menu", "Flash", "Welcome");
	...
	while (true) {
		...
		{
			Menu::Title("Unload?");
			if (Menu::Option("Yes")) g_running = false;
